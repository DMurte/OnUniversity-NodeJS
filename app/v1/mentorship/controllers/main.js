...
controller.getMentorship = async ((req, res) => {
    req.params.userId = req.session.userId;
    const validation = validate(req.params, validator.getMentorship, { format: 'flat' });
    if (validation) {
    ...
    try { 
        let response = await (queries.getMentorship(req.params));
        response = responses.successWithData(response);
        res.status(200).send(response);
